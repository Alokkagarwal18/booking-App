...
const HotelCard = ({ room, index }) => {
  ...
  return (
    <Link
      ...
      key={room._id}
      className="relative w-full sm:w-[300px] lg:w-full max-w-md rounded-xl overflow-hidden bg-white text-gray-500/90 shadow-md"
    >
      <img src={room.images?.[0]} alt={hotel.name || ''} className="w-full h-48 object-cover" />
      
      {index % 2 === 0 && <p className='px-3 py-1 absolute top-3 left-3 text-xs bg-white text-gray-800 font-medium rounded-full '>Best Seller</p>}

      <div className="p-4 pt-5">
        <div className="flex items-center justify-between">
          <p className="font-playfair text-xl font-medium text-gray-800">{hotel.name || 'Unknown Hotel'}</p>
          <div className="flex items-center gap-1">
